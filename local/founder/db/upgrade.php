...
function xmldb_local_founder_upgrade($oldversion) {
    global $DB;
    ...
    if ($oldversion < 2014060300) {
        ...
        $oldtable = new xmldb_table('local_founder_user_devices');
        ...
        if ($dbman->table_exists($newtable) and $dbman->table_exists($oldtable)) {
            ...
            if ($devices = $DB->get_records('local_founder_user_devices')) {
                $mappings = array();
        ...
        upgrade_plugin_savepoint(true, 2014060300, 'local', 'founder');
    }
    return true;
}